        ...
        <form @submit.prevent="submit" class="space-y-6">
            ...
            <div class="flex justify-end gap-2 pt-4">
                <UButton
                    ...
                    variant="soft"
                    label="Batal"
                    :to="`/dashboard/boarding-schools/${boardingSchool.id}`"
...
const form = useForm({
    name: props.boardingSchool.name,
    address: props.boardingSchool.address,
    description: props.boardingSchool.description || '',
    photo: null,
    // Ensure proper string array
    facilities: Array.isArray(props.boardingSchool.facilities)
        ? props.boardingSchool.facilities.map(f => String(f))
        : [],
});

// Photo handling
const photoPreview = ref(props.boardingSchool.photo_url);
...
const submit = () => {
    form.transform((data) => ({
        ...data,
        _method: 'PUT',
    })).post(`/dashboard/boarding-schools/${props.boardingSchool.id}`);
};
</script>
