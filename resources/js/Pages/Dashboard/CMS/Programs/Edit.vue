<template>
    <DashboardPage
        ...
        description="Ubah data program pondok"
        page-id="cms-programs-edit"
    >
        <template #header>
            <UButton
                color="neutral"
        ...
        <UCard>
            <UForm :state="form" @submit="submit" class="space-y-6 p-4 max-w-3xl mx-auto w-full">
                ...
                <UFormField label="Upload Gambar" name="image" hint="Format: JPG, PNG, WEBP (Max 2MB)">
                     <div class="space-y-3">
                        <!-- Current Image -->
                        <div v-if="!imagePreview && program.image" class="mb-2">
                             <img 
                                :src="program.image.startsWith('http') ? program.image : `/storage/${program.image}`" 
                                alt="Current Image" 
...
const submit = () => {
    form.transform((data) => ({
        ...data,
        _method: 'PUT',
    })).post(`/dashboard/cms/programs/${props.program.id}`, {
        forceFormData: true,
    });
};
</script>
