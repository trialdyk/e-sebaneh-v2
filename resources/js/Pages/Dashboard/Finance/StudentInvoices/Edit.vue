<template>
    <DashboardPage
        title="Edit Tagihan"
        heading="Edit Tagihan"
        :description="`Edit tagihan: ${invoice.name}`"
        page-id="edit-student-invoice"
    >
        <template #header>
            <UButton
                color="neutral"
                ...
                :to="`/dashboard/finance/student-invoices/${invoice.id}`"
            >
                Kembali
            </UButton>
        </template>
...
const form = useForm({
    ...
    for_gender: props.invoice.for_gender || 'male',
    classrooms: props.invoice.classrooms?.map(c => c.id) || [],
    students: props.invoice.students?.map(s => s.id) || []
});

const studentSearch = ref('');
const searchResults = ref([]);
const selectedStudents = ref(props.invoice.students?.map(s => ({
    id: s.id,
    name: s.user?.name + ' (' + s.student_id + ')' // Construct text similar to search result
})) || []);
...
function addStudent(student) {
    if (!form.students.includes(student.id)) {
        ...
        selectedStudents.value.push({ id: student.id, name: student.text });
    }
    studentSearch.value = '';
    searchResults.value = [];
}
...
function submitForm() {
    form.transform((data) => {
        ...
        return payload;
    }).put(`/dashboard/finance/student-invoices/${props.invoice.id}`, {
        onSuccess: () => {
